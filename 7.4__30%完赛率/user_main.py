...
stop_flag = 1
gyro_bias_x = -726          #-726
gyro_bias_y = 16.15          #16.15
gyro_bias_z = -140.56         #-140.56
# def get_offset():
...
#                     balance_angle = data_wave[i]
        ...
        wireless.send_oscilloscope(
             elementdetector.state, vel_ki, vel_kd, angle_kp, vel_disturbance, angle_disturbance, current_angle
#               elementdetector.state,current_angle, speed_slow_distance.data
        #     #imu_data[3], imu_data[4], imu_data[5]
        #     #turn_in_disturbance,turn_output, error
#              gyro_bias_x , gyro_bias_y, gyro_bias_z,imu_data[3],imu_data[4],imu_data[5]
             )
        gc.collect()
        ticker_flag_8ms = False
    gc.collect()
