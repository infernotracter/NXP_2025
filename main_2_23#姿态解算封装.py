...
class QuaternionFilter:
    ...
    def update(self, accel, gyro):
        ...
        # ---- 四元数积分 ----
        half_dt = 0.5 * self.dt
        q0_temp = (-self.q1*gx - self.q2*gy - self.q3*gz) * half_dt
        q1_temp = ( self.q0*gx +self.q2*gz - self.q3*gy) * half_dt
        q2_temp = ( self.q0*gy - self.q1*gz + self.q3*gx) * half_dt
        q3_temp = ( self.q0*gz + self.q1*gy - self.q2*gx) * half_dt
        ...
        # ---- 计算欧拉角 ----
        self.pitch = math.degrees(math.asin(2*(self.q0*self.q2 - self.q1*self.q3)))
...
while True:
    ...
    if (ticker_flag_3ms):
        ...
        imu_data = imu.get()  
        acc = [imu_data[0], imu_data[1], imu_data[2]]
        gyro = [imu_data[3], imu_data[4], imu_data[5]]

        # 更新滤波器
        imu_filter.update(acc, gyro)
    ...
    if (ticker_flag_50ms):
        angle_pid.pid_standard_integral(speed_pid.out + MedAngle, imu_filter.pitch)
        ticker_flag_50ms= False
