...
def elementdetector():
    ...
    global state, prev_state, tmperror, outflag, zebra_count
    tempcheck = state
...
def _left_1():
    global ccd_near_l,ccd_near_r,ccd_far_l_lost,ccd_far_r,POINT_diff_data
    near_valid = (ccd_near_l[0] <= ccd_near.left <= ccd_near_l[1] and 
                 ccd_near_r[0] <= ccd_near.right <= ccd_near_r[1])
    far_valid = (ccd_far.left < ccd_far_l_lost and 
                ccd_far_r[0] <= ccd_far.right <= ccd_far_r[1])
...
def _left_2():
    global ccd_near_l_lost,ccd_near_r,POINT_diff_data
...
def _left_3():
    global state , DISTANCE_ring3_data ,GYRO_Z_ring3_data,POINT_diff_data
...
def _left_in_not():
    global state , ccd_near_l_lost , ccd_near_r ,POINT_diff_data,DISTANCE_ring3_not_data
    near_left_lost = ccd_near.left <= ccd_near_l_lost
    near_right_valid = ccd_near_r[0] <= ccd_near.right <= ccd_near_r[1]
    point_diff = abs(ccd_far.right - ccd_near.right)
...
def _left_out_out():
    global DISTANCE_ring_out_out_data,ccd_near_l_lost,ccd_near_r_lost
...
def _left_out():
    global DISTANCE_ring_out_data
    return abs(element_distance.data) > DISTANCE_ring_out_data

def _crossroad():
    global ccd_near_l,ccd_near_r,ccd_far_l,ccd_far_r
