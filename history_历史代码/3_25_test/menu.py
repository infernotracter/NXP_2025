...
def sec_menu_01(key_data):
    global speed_flag, main_menu_flag, main_point_item, car_go_flag
...
def sec_menu_02(key_data):
    global speed_flag, main_menu_flag, main_point_item
    lcd.str24(60, 0, "speed", 0x07E0)
    lcd.str16(16, 126, "return", 0xFFFF)
    lcd.str16(16, 30, "aim_speed_l={}".format(sys.aim_speed_l), 0xFFFF)
    lcd.str16(16, 46, "aim_speed_r={}".format(sys.aim_speed_r), 0xFFFF)
    lcd.str16(16, 62, "motor_l.duty={}".format(motor_l.duty()), 0xFFFF)
    lcd.str16(16, 78, "motor_r.duty={}".format(motor_r.duty()), 0xFFFF)
    lcd.str16(16, 94, "encoder_l={:0>4d}".format(sys.encl_data), 0xFFFF)
    lcd.str16(16, 110, "encoder_r={:0>4d}".format(sys.encr_data), 0xFFFF)
    lcd.str16(0, main_point_item, ">", 0xF800)
...
def sec_menu_04(key_data):
    ...
    lcd.str16(16, 126, "medAngle={}".format(sys.MedAngle, '.2f'), 0xFFFF)
    lcd.str16(0, main_point_item, ">", 0xF800)
...
def sec_menu_07(key_data):
    global main_point_item, main_menu_flag, ccd_image_flag
...
def sec_menu_08(key_data):
    global main_point_item, parameter_flag
    lcd.str24(60, 0, "parameter", 0x07E0)  # 二级菜单标题
    lcd.str16(16, 30, "return", 0xFFFF)  # 返回主菜单

    lcd.str16(16, 46, "tof = {:<4d}".format(sys.tof_data), 0xFFFF)  # TOF数据
    lcd.str16(16, 62, "out_l = {:<4d}".format(sys.out_l), 0xFFFF)  # 左环pid输出
    lcd.str16(16, 78, "out_r = {:<4d}".format(sys.out_r), 0xFFFF)  # 右环pid输出
    lcd.str16(16, 94, "encl = {:<4d}".format(sys.encl_data), 0xFFFF)  # 左编码器值
    lcd.str16(16, 110, "encr = {:<4d}".format(sys.encr_data), 0xFFFF)  # 右编码器值
    lcd.str16(16, 126, "Mid_point1 = {:<.2f}".format(
        sys.mid_point1), 0xFFFF)  # ccd1中点
    lcd.str16(16, 142, "Mid_point2 = {:<.2f}".format(
        sys.mid_point2), 0xFFFF)  # ccd2中点
    lcd.str16(16, 158, "error1 = {:<.2f}".format(sys.error1), 0xFFFF)  # ccd1误差
    lcd.str16(16, 174, "error2 = {:<.2f}".format(sys.error2), 0xFFFF)  # ccd2误差
    lcd.str16(16, 190, "left_point_1 = {:<3d}".format(
        sys.left_point_1), 0xFFFF)  # 上摄像头左边点
    lcd.str16(16, 206, "right_point_1 = {:<3d}".format(
        sys.right_point_1), 0xFFFF)  # 上摄像头右边点
    lcd.str16(16, 222, "left_point_2 = {:<3d}".format(
        sys.left_point_2), 0xFFFF)  # 下摄像头左边点
    lcd.str16(16, 238, "right_point_2 = {:<3d}".format(
        sys.right_point_2), 0xFFFF)  # 下摄像头右边点
    lcd.str16(16, 254, "width_1 = {:<3d}".format(
        sys.right_point_1 - sys.left_point_1), 0xFFFF)  # ccd1计算赛道宽度
    lcd.str16(16, 270, "width_2 = {:<3d}".format(
        sys.right_point_2 - sys.left_point_2), 0xFFFF)  # ccd2计算赛道宽度
