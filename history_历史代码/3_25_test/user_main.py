...
while True:
    ...
    if end_switch.value() == 1:
        ticker.pit1.stop()  # pit1关闭
        ticker.pit2.stop()  # pit2关闭
        ticker.pit3.stop()  # pit3关闭
        break  # 跳出判断

    # 1ms中断标志位
    if (ticker.ticker_flag_1ms):
        sys.imu_filter()
        ...
        ticker.ticker_flag_1ms = False

    if (ticker.ticker_flag_5ms):
        sys.update_encoder()
        # 原函数此时为圆环处理
        ticker.ticker_flag_5ms = False

    if (ticker.ticker_flag_2ms):  # kp=100.1  ki=2.0000001
        pid.gyro_pid_out = gyro_pid.calculate(pid.angle_pid_out, sys.imu_data[3])
        ticker.ticker_flag_2ms = False

    if (ticker.ticker_flag_10ms):
        pid.angle_pid_out = angle_pid.calculate(
            pid.speed_pid_out + sys.MedAngle, sys.current_roll)
        menu(key_data)
        key_data = key.get()
        ticker.ticker_flag_10ms = False

    if (ticker.ticker_flag_50ms):
        pid.speed_pid_out = speed_pid.calculate(
            sys.aim_speed, (sys.encl_data + sys.encr_data) / 2)
        ticker.ticker_flag_50ms = False
