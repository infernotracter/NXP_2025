...
class speed_ring:
    ...
    def pid_standard_integral(self, aim_speed, speed):
        ...
        self.out = self.kp * self.err + self.increment+self.kd * (self.err-self.err_last)
...
class angle_ring:
    ...
    def pid_standard_integral(self, aim_angle, angle):
        ...
        self.increment += self.err * self.ki
        my_limit(self.increment, -2000, 2000)
        self.out = self.kp * self.err + self.increment+self.kd * (self.err - self.err_last)
        self.err_last = self.err
        return self.out
...
def search_element():
    ...
    # 远端左边丢线状态
    if (left_point_1 < 7) and (30 <= left_point_2 <= 42) and (79 <= right_point_1 <= 99) and (
            abs(right_point_1 - right_point_2) <= 10):
        n = 1
    # 近端左边丢线，用右边线补左线
    ...
    # 远端右边丢线状态
    if (right_point_1 > 108) and (90 <= right_point_2 <= 108) and (29 <= left_point_1 <= 49) and (
            abs(left_point_1 - left_point_2) <= 10):
        m = 1
    # 近端右边丢线，用左边线补右线
