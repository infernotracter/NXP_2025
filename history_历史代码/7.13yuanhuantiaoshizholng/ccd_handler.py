...
class ElementDetector:
    ...
    def __init__(self):
        ...
        self.GYRO_Z_ring2_data = 650

        # l3
        self.GYRO_Z_ring3_data = 2000
        self.DISTANCE_ring3_data = 100

        # lin
        self.GYRO_Z_ring_in_data = 6000
        self.DISTANCE_ring_in_data = 500 #150
...
#         if  abs(element_distance.data)>600 and (self.state != RoadElement.lin) :
#             self.state = RoadElement.normal
...
#                 self.state = RoadElement.normal
    ...
    def _element_operations(self):
        ...
        elif self.state == RoadElement.l0:
            ccd_controller.follow = -self.ccd_near_length
        elif self.state == RoadElement.l1:
            ccd_controller.follow = -self.ccd_near_length
        elif self.state == RoadElement.l2:
...
#             return 0      #没检测到障碍物
    ...
    def _crossroad_coming(self):
        near_valid = (ccd_near.left > self.ccd_near_l_lost) and (ccd_near.right < self.ccd_near_r_lost)
...
def is_circus():
    ...
    circus_in=(elementdetector.state==RoadElement.lin or elementdetector.state==RoadElement.rin or elementdetector.state==RoadElement.loutcoming or elementdetector.state==RoadElement.routcoming)
    circus_out=(elementdetector.state==RoadElement.lout or elementdetector.state==RoadElement.rout or elementdetector.state==RoadElement.loutout or elementdetector.state==RoadElement.routout)
