...
def search_element():
    ...
    # 左圆环
    if (20 <= left_point_2 <= 38) and (85 <= right_point_2 <= 107):      # 判断近端ccd左右无丢线
        if (left_point_1 < 5) and (81 <= right_point_1 <= 100):          # 判断远端ccd左丢线
            if (abs(right_point_1 - right_point_2 ) <= 15):              # 判断两个ccd不丢线一端是否变化
                flag = road_type['左圆环1']                               # 标志位1
    #判断两个ccd不丢线一端是否变化,且近端ccd左丢线
    if (abs(right_point_1 - right_point_2 ) <= 15) and (left_point_2 <= 10) and (85 <= right_point_2 <= 105):
    ...
    # 右圆环
    if (20 <= left_point_2 <= 38) and (85 <= right_point_2 <= 107):      # 判断近端ccd左右无丢线
        if (20 <= left_point_1 <= 45) and (108 <= right_point_1):        # 判断远端ccd右丢线
            if (abs(left_point_1 - left_point_1 ) <= 15):                # 判断两个ccd不丢线一端是否变化
                flag = road_type['右圆环1']                               # 标志位1
    #判断两个ccd不丢线一端是否变化,且近端ccd右丢线
    if (abs(left_point_1 - left_point_2) <= 15) and (108 <= right_point_2) and (20 <= left_point_2 <= 38):
...
speed_pid_r = motor_PID(kp_motor = 10.0, ki_motor = 0.6, kd_motor = 0)    # 右电机PID初始化
servo_pid = turn_PID(kp_turn = 0.435, kd_turn = 2.3)                      # 舵机PID初始化
T = 0     # 总路程
while True:
    # 计算路程
    ...
    # 速度控制    
    if (error2 >= 10.0):
        aim_speed_l = T #- (servo_pid.Direction_pid(error2) * 4.5)   # 差速：目标速度 +/- (舵机输出 * 差速系数)
        aim_speed_r = T #+ (servo_pid.Direction_pid(error2) * 6.5)   # 差速：目标速度 +/- (舵机输出 * 差速系数)
    if (-10.0 <= error2 <= 10.0):
        aim_speed_l = T     # 无差速状态
        aim_speed_r = T     # 无差速状态
    if (error2 <= -10.0):
    ...
    if (ticker_flag):
        ...
        # 左圆环处理
        if (flag is '右圆环1') and m == 0 and (distance_T >= 1.3):
            m = 1
        ...
        if (m == 2):
            flag is not  '右圆环1'
            # 计算第一个路口距离(距离是固定的)
            distance_r_1 += (encl_data + encr_data) / 2 / 1024 * 30 / 50 * 0.05 * 3.1415926
            if(distance_r_1 >= 0.28 ):
                m = 3
                M = 2
                distance_l_1 = 0
        if (m == 3):
        ...
        if (m == 4):
            grzo_z = 0
            # 计算补线距离仍是一个路口距离
            distance_r_2 += (encl_data + encr_data) / 2 / 1024 * 30 / 50 * 0.05 * 3.1415926
            if(distance_r_2 >= 0.35):
                m = 5
                M = 0
                distance_l_2 = 0
