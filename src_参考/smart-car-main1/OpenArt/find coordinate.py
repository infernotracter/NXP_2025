...
def find_coordinates(img):
    ...
    for r in img.find_rects(threshold=3000):
        ...
        if r.w() > 120 and r.w() < 180 and r.h() > 70 and r.h() < 130:
            ...
            corners = r.corners()
            img1 = img.rotation_corr(0,0,0,0,0,1,60,corners)
            img1.draw_rectangle(r.rect(), color=(255, 0, 0))
            # 将矩形属性打印出来
            # jiangly 不是dd
            # print("w:" + str(r.w()) + "  h:" + str(r.h()))
            # 在矩形中搜索圆形
            for c in img1.find_circles(threshold=2800, x_margin=5, y_margin=5, r_margin=5, r_min=1,
                                      r_max=5, r_step=1):
                # 绘制圆形外框
