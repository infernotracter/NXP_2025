...
#define FLEXIO_PIN_CONF     SPEED_100MHZ | KEEPER_EN | DSE_R0_6 //配置SPI引脚默认配置
...
{
    uint8 pin[32]={B31,B30,B29,B28,B27,B26,B25,B24,B23,B22,B21,B20,B19,B18,B17,B16,C4,C5,C6,C7,C8,C9,C18,C19,C20,C21,C22,C23,C24,C25,C26,C27};
...
void flexio_camera(FLEXIO_PIN_enum dat_pin_start, FLEXIO_PIN_enum pclk_pin, FLEXIO_PIN_enum href_pin, uint16 col, uint16 row)
{
    ...
    s_FlexioCameraDevice.flexioBase = FLEXIO1;                  //设置基地址
    ...
    s_FlexioCameraDevice.shifterStartIdx = 8-flexio_shift_count;   //设置环移器起始编号
    s_FlexioCameraDevice.shifterCount = flexio_shift_count;        //设置缓冲区数量
    s_FlexioCameraDevice.timerIdx = 7;                          //设置定时器编号
