...
//如果不定义在noncache区域，则会造成摄像头数据采集完成后，内核去访问这个数组的时候，读取的数据是直接从cache里面读取的而不是去获取更新后的内容
//从而造成不同步的问题。

//V1.05版本不在单独指定将变量放入noncache区域，默认将变量放入DTCM区域，DTCM为noncache区域与主频 同频
AT_DTCM_SECTION_ALIGN(uint8 image[MT9V032_H][MT9V032_W], 4);
...
void camera_init(void)
{
    ...
    NVIC_SetPriority(MT9V032_VSYNC_IRQN,0);         //设置场中断优先级 范围0-15 越小优先级越高
	//EnableGlobalIRQ(0);
}
