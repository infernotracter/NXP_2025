...
{
    float32 clockFreq = IfxGtm_Tom_Ch_getClockFrequency(driver->gtm, driver->tom, driver->timerChannel);
    uint32  divider   = ((driver->gtm->CLS_CLK_CFG.U >> (driver->tomIndex * 2)) & 0x3);

    if (divider != 0)
    {
        clockFreq = clockFreq / divider;
    }

    driver->base.clockFreq = clockFreq;
}
