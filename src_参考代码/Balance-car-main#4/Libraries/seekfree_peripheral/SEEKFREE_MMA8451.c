...
void get_mma8451(void)
{
	...
	//补码求出加速度原始对应数值
	if(dat[0]>0x7f)
		acc_x=-(int16)((~(wx>>2) + 1)&0X3FFF);								// 移位取反加一再去掉无效字符
	else
		acc_x=(wx>>2)&0X3FFF;												// 移位掉无效字符


	wy = (uint16)dat[2]<<8 | dat[3];
	//补码求出加速度原始对应数值
	if(dat[2]>0x7f)
		acc_y=-(int16)((~(wy>>2) + 1)&0X3FFF);								// 移位取反加一再去掉无效字符
	else
		acc_y=(wy>>2)&0X3FFF;												// 移位掉无效字符
