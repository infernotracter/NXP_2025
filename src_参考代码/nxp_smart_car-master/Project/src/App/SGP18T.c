...
【注意事项】
----------------------------------------------------------------*/  
void TFTSPI_Show_Cmera(uint8_t xs,uint8_t ys,uint8_t w,uint8_t h,u8 Pixle[LCDH][LCDW]) 
{
 #ifdef GRAY    //灰度显示
   unsigned int i,j; 
   uint16_t color;
   ...
   {   
       ...
       {
           color = 0;
           color=((Pixle[i][j]>>3))<<11;
           color=color|(((Pixle[i][j]>>2))<<5);
           color=color|(((Pixle[i][j])>>3));
           TFTSPI_Write_Word(color);
       }
